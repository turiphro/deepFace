...
function<ofVec2f(ofVec2f)> createCenterForce(ofVec2f center) {
    return [center] (ofVec2f pos) -> ofVec2f {
        ...
        if (distance > radiusInner) {
            return difference * (distance - radiusInner) / ofGetWidth(); } else {
            return zero;
        }
    };
}

function<ofVec2f(ofVec2f)> createCircleForce(ofVec2f center) {
    return [center] (ofVec2f pos) -> ofVec2f {
